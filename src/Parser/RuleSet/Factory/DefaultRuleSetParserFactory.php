...
final class DefaultRuleSetParserFactory implements RuleSetParserFactoryInterface
{
    private const RULE_SET_PARSERS = [
        RuleSetParser::ARRAY->value,
        RuleSetParser::DEFAULT->value,
        RuleSetParser::REGEXP->value,
    ];
    ...
    public function getRuleSetParser(
        ...
        if (!isset($this->ruleSetParsers[$type])) {
            $this->ruleSetParsers[$type] = $this->resolveRuleSetParser($type, $nodeParser, $nodeMapper);
        }

        return $this->ruleSetParsers[$type];
    }

    private function resolveRuleSetParser(
        string $type,
        ?NodeParserInterface $nodeParser,
